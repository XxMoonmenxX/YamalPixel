...
def check_for_updates():
    try:
        ...
        if latest_version != CURRENT_VERSION:
            ...
            if answer:
                download_and_install_update(response.json()['assets'][0]['browser_download_url'])
        else:
...
def download_and_install_update(download_url):
    try:
        ...
        update_file = os.path.join(os.getcwd(), "launcher_update.zip")
        ...
        with requests.get(download_url, stream=True) as r:
            ...
            with open(update_file, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
                    update_progress(f.tell(), 8192, total_size)
        ...
        # Перезапуск
        logging.info("Обновление успешно установлено")
        messagebox.showinfo("Успех", "Лаунчер успешно обновлен! Перезапускаем...")
