...
CURRENT_VERSION = "0.5.1"  # обновление
logging.basicConfig(
...
SHADERS_CONFIG = {
    "shaders": [
        {
            "name": "Aurora Shaders",
            "url": "https://disk.yandex.ru/d/Ish63cvEZjqqMw",
            ...
            try:
                win.after(
                    ...
                    lambda: update_progress(
                        (i * 100) // total, f"Скачивание: {shader['name']}..."
                    ),
                )
                ...
                if direct_link:
                    ...
                    logging.error(
                        f"Не удалось получить ссылку для шейдера: {shader['name']}"
                    )
...
def auto_repair_game_files(silent=True):  # 👈 ДОБАВЛЯЕМ SILENT ПАРАМЕТР
    ...
    # Заголовок
    ttk.Label(
        main_frame, text="🔧 Автопочинка файлов игры", font=("Comfortaa", 16, "bold")
    ).pack(pady=(0, 10))
    ...
    # Кнопка отмены
    cancel_btn = ttk.Button(
        main_frame, text="❌ Отменить починку", state="disabled"
    )  # Пока отключена
    cancel_btn.pack(pady=10)
        ...
        try:
            ...
            if not os.path.exists(servers_file):
                ...
                try:
                    ...
                    add_log(
                        f"⚠️ Не удалось восстановить servers.dat: {str(e)}", "orange"
                    )
                    ...
                    try:
                        ...
                        if download_url:
                            ...
                            with open(mod_path, "wb") as f:
                                ...
                                total_size = int(
                                    dl_response.headers.get("content-length", 0)
                                )
                                downloaded = 0
                            ...
                            if mod["file"].endswith(".zip"):
                                try:
                                    ...
                                    fixes_applied.append(
                                        f"Загружен и распакован {mod['file']}"
                                    )
                                    add_log(
                                        f"✅ Загружен и распакован {mod['file']}",
                                        "green",
                                    ...
                                    fixes_applied.append(
                                        f"Загружен {mod['file']} (ошибка распаковки)"
                                    )
                                    add_log(
                                        f"⚠️ Загружен {mod['file']} (ошибка распаковки: {e})",
                                        "orange",
                                    )
                            else:
                        ...
                        add_log(
                            f"❌ Ошибка загрузки мода {mod['file']}: {str(e)}", "red"
                        )
            ...
            win.after(
                0, lambda: details_label.config(text="Проверяем установку Fabric")
            )
            ...
            win.after(
                0, lambda: details_label.config(text="Проверяем версию Minecraft")
            )
            ...
            if not os.path.exists(minecraft_version_dir):
                issues_found.append(
                    f"Версия Minecraft {CONFIG['version']} не установлена"
                )
                add_log(
                    f"❌ Версия Minecraft {CONFIG['version']} не установлена", "red"
                )
                try:
                    ...
                    fixes_applied.append(
                        f"Установлена версия Minecraft {CONFIG['version']}"
                    )
                    add_log(
                        f"✅ Установлена версия Minecraft {CONFIG['version']}", "green"
                    )
                except Exception as e:
...
def repair_game_with_options():
    ...
    title_label = ttk.Label(
        choice_window, text="Выберите действие:", font=("Comfortaa", 14, "bold")
    )
    ...
    ttk.Button(choice_window, text="❌ Отмена", command=cancel, width=20).pack(pady=20)
...
def checker1():
    ...
    if (
        version_selector.get() != "YamalPixel"
    ):  # ИСПРАВЛЕНО: version_combobox -> version_selector
...
def runn():
    ...
    try:
        ...
        auto_repair_game_files(
            silent=False
        )  # ИЗМЕНЕНО: silent=False для загрузки модов
...
def auto_pre_launch_check():
    ...
    actual_mods = len(
        [
            f
            for f in os.listdir(os.path.join(CONFIG["minecraft_dir"], "mods"))
            if f.endswith(".jar")
        ]
    )
