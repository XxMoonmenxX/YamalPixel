...
def runn():
    ...
    def install_minecraft():
        try:
            ...
            if not minecraft_launcher_lib.utils.is_version_valid(
                ver, minecraft_directory
            ):
                minecraft_launcher_lib.install.install_minecraft_version(
                    ver, minecraft_directory, callback=callback
                )
            ...
            options = {
                ...
                "jvmArguments": [
                    ...
                    f"-cp",
                    minecraft_launcher_lib.command.get_classpath(
                        ver, minecraft_directory
                    ),
    ...
    threading.Thread(
        target=install_minecraft
    ).start()  # Запускаем установку в отдельном потоке
...
app.configure(
    "TButton", font=("Comfortaa", 12)
)  # Задаем шрифт и размер для всех кнопок


username = ttk.Entry(win, style="BW.TLabel", width=20)
username.place(relx=0.5, rely=0.45, anchor="c")
...
style.configure(
    "CenterText.TLabel", layout=("Center",)
)  # Задаем параметры для центрирования текста
...
def update_online_button():
    btn_update_online = ttk.Button(
        win, text="Показать онлайн", style="BW.TLabel", command=show_online_players
    )
    btn_update_online.place(relx=0.5, rely=0.58, width=150, height=25, anchor="c")
