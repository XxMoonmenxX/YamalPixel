...
class ParallelDownloader:
    ...
    def download_multiple(self, download_list):
        """Параллельная загрузка списка файлов"""
        results = {}

        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Создаем задачи
            future_to_url = {}
            for url, filepath in download_list:
                future = executor.submit(self._download_single, url, filepath)
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    ...
                    results[(url, filepath)] = result
                except Exception as e:
                    print(f"❌ Ошибка загрузки {url[:50]}: {e}")
                    results[(url, filepath)] = False

        return results

    def _download_single(self, url, filepath):
        """Загрузка одного файла"""
        return download_file_through_proxy(url, filepath)


# Использование в патчинге minecraft_launcher_lib
def patch_for_parallel_downloads():
    """Патчим minecraft_launcher_lib для параллельной загрузки"""
    import minecraft_launcher_lib.install
    import minecraft_launcher_lib._helper

    # Создаем экземпляр параллельного загрузчика
    downloader = ParallelDownloader(max_workers=3)  # 3 одновременных загрузки

    # Патчим функцию install_minecraft_version
    original_install = minecraft_launcher_lib.install.install_minecraft_version

    def patched_install(*args, **kwargs):
        print("🎯 Используем параллельную загрузку для Minecraft")

        # Временно заменяем download_file на параллельную версию
        original_download = minecraft_launcher_lib.install.download_file

        def parallel_download(url, path, **download_kwargs):
            print(f"🔀 Параллельная загрузка: {os.path.basename(path)}")
            return download_file_through_proxy(url, path)

        minecraft_launcher_lib.install.download_file = parallel_download
        ...
        try:
            ...
            minecraft_launcher_lib.install.download_file = original_download

    minecraft_launcher_lib.install.install_minecraft_version = patched_install
    print("✅ Установлен патч для параллельной загрузки Minecraft")


def download_file_through_proxy_fast(original_url: str, filepath: str) -> bool:
    """
    Быстрая версия загрузки через прокси.
    """
    try:
        encoded_url = requests.utils.quote(original_url, safe='')
        proxy_url = f"{PROXY_BASE_URL}/api/v1/mirror/download/fast?url={encoded_url}"
...
# Используйте эту функцию в патчах minecraft_launcher_lib
def patch_minecraft_downloads_fast():
    """Патчим для использования быстрой загрузки"""
    import minecraft_launcher_lib._helper
    import minecraft_launcher_lib.install

    def fast_download_wrapper(url, path, *args, **kwargs):
        print(f"⚡ БЫСТРАЯ ЗАГРУЗКА: {os.path.basename(path)}")

        if download_file_through_proxy_fast(url, path):
            ...
            print(f"🔄 Fallback на обычную загрузку: {os.path.basename(path)}")
            if hasattr(minecraft_launcher_lib.install, 'download_file'):
                return minecraft_launcher_lib.install.download_file(url, path, **kwargs)
            return False

    # Патчим основные функции
    if hasattr(minecraft_launcher_lib._helper, 'download_file'):
        minecraft_launcher_lib._helper.download_file = fast_download_wrapper

    if hasattr(minecraft_launcher_lib.install, 'download_file'):
        minecraft_launcher_lib.install.download_file = fast_download_wrapper

    print("✅ Установлен патч для быстрой загрузки")
...
def download_direct_fallback(original_url: str, filepath: str) -> bool:
    """
    Fallback функция: скачивает файл напрямую если прокси недоступен
    И ОДНОВРЕМЕННО сообщает прокси о файле для кэширования
    """
    try:
        print(f"🔄 Прямая загрузка: {original_url[:80]}...")

        response = requests.get(original_url, stream=True, timeout=60)
        ...
        # Сообщаем прокси о файле для кэширования (в фоне, не блокируем)
        try:
            cache_request_url = f"{PROXY_BASE_URL}/api/v1/cache/request"
            cache_data = {
                "url": original_url,
                "filename": os.path.basename(filepath),
                "source": "fallback_download",
            ...
            def send_cache_request():
                try:
                    requests.post(cache_request_url, json=cache_data, timeout=5)
                    print(f"📨 Уведомили прокси о новом файле")
                except:
                    pass  # Не критично

            threading.Thread(target=send_cache_request, daemon=True).start()

        except Exception as cache_error:
            print(f"⚠️ Не удалось уведомить прокси: {cache_error}")

        return True

    except Exception as e:
        print(f"❌ Прямая загрузка тоже не удалась: {e}")
        return False


# ======================= ПОДМЕНА СТАНДАРТНЫХ ФУНКЦИЙ =======================
...
def download_file_through_proxy_wrapper(url, path, original_func, **kwargs):
    """
    Универсальная обертка для проксирования загрузок
    """
    ...
    if download_file_through_proxy(url, path):
        ...
        print(f"🔄 Используем оригинальную загрузку: {os.path.basename(path)}")
        return original_func(url, path, **kwargs)





# ======================= АВТОМАТИЧЕСКИЙ ПАТЧ ПРИ ЗАПУСКЕ =======================
# Вызываем патчинг сразу при импорте
patch_minecraft_downloads()

# Проверка в лаунчере
cf_loader = CurseForgeAPI("http://90.151.59.120:8000")
mr_loader = ModrinthAPI("http://90.151.59.120:8000")
...
def install_required_components_sync(version_name):
    ...
    try:
        ...
        if version_name.startswith("📦 "):
            ...
            print(f"📋 Конфигурация: {config_data}")
            ...
            if isinstance(config_data, (list, tuple)):
                if len(config_data) == 2:
                    ...
                    raise ValueError(
                        f"Неверная конфигурация для {version_name}: ожидалось 2 или 3 значения, получено {len(config_data)}"
                    )
            else:
                # Если это строка (например, просто версия Minecraft)
        ...
        if not version_exists:
            ...
            try:
                # ОЧЕНЬ ВАЖНО: Используем правильные параметры
                minecraft_launcher_lib.install.install_minecraft_version(
                    version=minecraft_version,  # Правильный параметр
                    minecraft_directory=CONFIG["minecraft_dir"]
                )
                print(f"✅ Minecraft {minecraft_version} успешно установлена")
            except Exception as e:
                print(f"❌ Ошибка установки Minecraft {minecraft_version}: {e}")
                # Пробуем альтернативный метод
                return install_minecraft_version_with_retry(minecraft_version)

        else:
            print(f"✅ Minecraft {minecraft_version} уже установлена")

        # 3. УСТАНАВЛИВАЕМ МОДЛОАДЕР (ЕСЛИ НУЖЕН)
